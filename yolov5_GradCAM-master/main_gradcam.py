...
parser.add_argument('--model-path', type=str, default="weights/mask.pt", help='Path to the model')
parser.add_argument('--img-path', type=str, default='data/images/mask.jpg', help='input image path')
...
# get_res_img:将热力图与原图进行alpha blending
def get_res_img(bbox, mask, res_img):
    # mask（即一个物体的saliency_map）之前是被归一化过的，故要乘255还原为像素值
...
def main(img_path):
    ...
    for target_layer in target_layers:
        ...
        # 将图像的batch维度去掉，同时将图像变换到[0, 255]，最后将颜色通道放到最后一个维度
        ...
        for i, mask in enumerate(masks):
            ...
            # 获取目标alpha blending后的热力图
            res_img, heat_map = get_res_img(bbox, mask, res_img)
            # 是否需要画预测框
            if not args.no_text_box:
